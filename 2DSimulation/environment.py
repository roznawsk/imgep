...
class GRBFTrajectory(object):
    def __init__(self, n_dims, sigma, steps_per_basis, max_basis):
        self.n_dims = n_dims
        self.sigma = sigma
        self.alpha = -1.0 / (2.0 * self.sigma**2.0)
        ...
        for i in range(2 * self.max_basis * self.steps_per_basis):
            self.precomputed_gaussian[i] = self.gaussian(
                self.max_basis * self.steps_per_basis, i
            )

    def gaussian(self, center, t):
        return np.exp(self.alpha * (center - t) ** 2.0)
    ...
    def trajectory(self, weights):
        ...
        for step in range(steps):
            g = self.precomputed_gaussian[
                self.max_basis * self.steps_per_basis
                + self.steps_per_basis
                - 1
                - step :: self.steps_per_basis
            ][:n_basis]
...
class ArmToolsToysEnvironment(Environment):
    ...
    def __init__(self, rdm_distractors=True):
        ...
        self.trajectory_generator = GRBFTrajectory(
            self.motor_dims, self.sigma, self.steps_per_basis, self.max_basis
        )

        self.m_mins = [-1.0] * self.motor_dims * self.max_basis
        self.m_maxs = [1.0] * self.motor_dims * self.max_basis
        ...
        self.arm_rest_state = [0.0, 0.0, 0.0, 0.0]

        # Stick1
        self.stick1_length = 0.5
        self.stick1_type = "magnetic"
        self.stick1_handle_tol = 0.03
        self.stick1_handle_tol_sq = self.stick1_handle_tol**2.0
        ...
        self.stick2_handle_tol_sq = self.stick1_handle_tol**2.0
        self.stick2_rest_state = [0.75, 0.25, 0.25]

        # Magnet1
        self.magnet1_tolsq = 0.01**2.0
        self.magnet1_rest_state = [-0.3, 1.1]
        # Magnet2
        self.magnet2_tolsq = 0.0
        self.magnet2_rest_state = [-0.5, 1.5]
        # Magnet3
        self.magnet3_tolsq = 0.0
        self.magnet3_rest_state = [-0.3, 1.5]

        # Scratch1
        self.scratch1_tolsq = 0.01**2.0
        self.scratch1_rest_state = [0.3, 1.1]
        # Scratch2
        self.scratch2_tolsq = 0.0
        self.scratch2_rest_state = [0.3, 1.5]
        # Scratch3
        self.scratch3_tolsq = 0.0
        ...
        # Static objects
        self.static_objects_rest_state = [
            [-0.7, 1.1],
            [-0.5, 1.1],
            [0.5, 1.1],
            [0.7, 1.1],
    ...
    def reset(self):
        ...
        self.hand_pos = np.array(
            [
                np.sum(np.cos(a_pi) * self.arm_lengths),
    ...
    def step(self, m):
        ...
        a_pi = [np.pi * a[0], np.pi * a[1], np.pi * a[2]]
        ...
        if m[-1] >= 0.0:
            ...
            new_gripper = -1.0
        gripper_change = (self.gripper - new_gripper) / 2.0
        ...
        if not self.stick1_held:
            if (
                gripper_change == 1.0
                and (self.hand_pos[0] - self.stick1_handle_pos[0]) ** 2.0
                + (self.hand_pos[1] - self.stick1_handle_pos[1]) ** 2.0
        ...
        if not self.stick2_held:
            if (
                gripper_change == 1.0
                and (self.hand_pos[0] - self.stick2_handle_pos[0]) ** 2.0
                + (self.hand_pos[1] - self.stick2_handle_pos[1]) ** 2.0
        ...
        if (
            self.magnet1_move == 1
            or (self.stick1_end_pos[0] - self.magnet1_pos[0]) ** 2
            + (self.stick1_end_pos[1] - self.magnet1_pos[1]) ** 2
            < self.magnet1_tolsq
        ):
        ...
        if (
            ...
            or (self.stick2_end_pos[0] - self.scratch1_pos[0]) ** 2
            + (self.stick2_end_pos[1] - self.scratch1_pos[1]) ** 2
            < self.scratch1_tolsq
        ):
    ...
    def render(self, close=False):
        ...
        x = np.hstack((0.0, np.cumsum(np.cos(a) * self.arm_lengths)))
        y = np.hstack((0.0, np.cumsum(np.sin(a) * self.arm_lengths)))
        ...
        if self.stick1_held or self.n_step <= 1:
            self.lines["s11"][0].set_data(
                [self.stick1_handle_pos[0], self.stick1_end_pos[0]],
                [self.stick1_handle_pos[1], self.stick1_end_pos[1]],
            )
            self.lines["s12"][0].set_data(
                self.stick1_handle_pos[0], self.stick1_handle_pos[1]
            )
            self.lines["s13"][0].set_data(
                self.stick1_end_pos[0], self.stick1_end_pos[1]
            )

        # Magnet1
        self.patches["mag1"].set_xy(
            (self.magnet1_pos[0] - 0.05, self.magnet1_pos[1] - 0.05)
        )

        # Stick2
        if self.stick2_held or self.n_step <= 1:
            self.lines["s21"][0].set_data(
                [self.stick2_handle_pos[0], self.stick2_end_pos[0]],
                [self.stick2_handle_pos[1], self.stick2_end_pos[1]],
            )
            self.lines["s22"][0].set_data(
                self.stick2_handle_pos[0], self.stick2_handle_pos[1]
            )
            self.lines["s23"][0].set_data(
                self.stick2_end_pos[0], self.stick2_end_pos[1]
            )

        # Scratch1
        self.patches["scr1"].set_xy(
            (self.scratch1_pos[0] - 0.05, self.scratch1_pos[1] - 0.05)
        )

        # Cat
        self.patches["cat"].set_xy((self.cat_pos[0] - 0.05, self.cat_pos[1] - 0.05))
        # Dog
        self.patches["dog"].set_xy((self.dog_pos[0] - 0.05, self.dog_pos[1] - 0.05))
    ...
    def start_viewer(self):
        ...
        ax.axis([-2, 2.0, -1.5, 2.0])

        # Gripper
        self.lines["g1"] = ax.plot(
            3,
            3,
            "o",
        ...
        self.lines["s12"] = ax.plot(
            self.stick1_handle_pos[0], self.stick1_handle_pos[1], "o", color="g", ms=6
        )
        self.lines["s13"] = ax.plot(
            self.stick1_end_pos[0], self.stick1_end_pos[1], "o", color="b", ms=6
        )

        # Stick2
        self.lines["s21"] = ax.plot(
            [self.stick2_handle_pos[0], self.stick2_end_pos[0]],
            [self.stick2_handle_pos[1], self.stick2_end_pos[1]],
            "-",
            color="grey",
        ...
        self.lines["s22"] = ax.plot(
            self.stick2_handle_pos[0], self.stick2_handle_pos[1], "o", color="g", ms=6
        )
        self.lines["s23"] = ax.plot(
            self.stick2_end_pos[0], self.stick2_end_pos[1], "o", color="c", ms=6
        )

        # Magnet1
        p = plt.Rectangle(
            (self.magnet1_pos[0] - 0.05, self.magnet1_pos[1] - 0.05), 0.1, 0.1, fc="b"
        )
        self.patches["mag1"] = p
        ax.add_patch(p)
        # Magnet2
        ax.add_patch(
            plt.Rectangle(
                (self.magnet2_pos[0] - 0.05, self.magnet2_pos[1] - 0.05),
                0.1,
                0.1,
                fc="b",
            )
        ...
        ax.add_patch(
            plt.Rectangle(
                (self.magnet3_pos[0] - 0.05, self.magnet3_pos[1] - 0.05),
                0.1,
                0.1,
                fc="b",
            )
        ...
        # Scratch1
        p = plt.Rectangle(
            (self.scratch1_pos[0] - 0.05, self.scratch1_pos[1] - 0.05), 0.1, 0.1, fc="c"
        )
        ax.add_patch(p)
        self.patches["scr1"] = p
        # Scratch2
        ax.add_patch(
            plt.Rectangle(
                (self.scratch2_pos[0] - 0.05, self.scratch2_pos[1] - 0.05),
                0.1,
                0.1,
                fc="c",
            )
        ...
        ax.add_patch(
            plt.Rectangle(
                (self.scratch3_pos[0] - 0.05, self.scratch3_pos[1] - 0.05),
                0.1,
                0.1,
                fc="c",
            )
        ...
        # Cat
        p = plt.Rectangle(
            (self.cat_pos[0] - 0.05, self.cat_pos[1] - 0.05), 0.1, 0.1, fc="m"
        )
        ax.add_patch(p)
        self.patches["cat"] = p
        # Dog
        p = plt.Rectangle(
            (self.dog_pos[0] - 0.05, self.dog_pos[1] - 0.05), 0.1, 0.1, fc="y"
        )
        ax.add_patch(p)
        self.patches["dog"] = p
        ...
        for pos in self.static_objects_pos:
            ax.add_patch(
                plt.Rectangle((pos[0] - 0.05, pos[1] - 0.05), 0.1, 0.1, fc="k")
            )
