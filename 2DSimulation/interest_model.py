...
class MiscRandomInterest(RandomInterest):
    def __init__(self, conf, expl_dims, n_sdims, win_size=100):
        ...
        self.data = Dataset(len(expl_dims), len(expl_dims), max_size=1000)
    ...
    def competence_dist(self, target, reached):
        ...
        if len(reached) < len(target):
            augm = np.tile(
                reached[-self.n_sdims :], (len(target) - len(reached)) // self.n_sdims
            )
    ...
    def update(self, sg, s, log=False):
        if len(self.data) > 0:
            ...
            if log:
                ...
                print("c", 0.01 * int(c * 100.0))
                print("c_old", 0.01 * int(c_old * 100.0))
                print("progress", 0.01 * int(progress * 100.0))
                # print("data", self.data.data)
        else:
            progress = 0.0
        # Update progress and interest
        self.current_progress += (1.0 / self.win_size) * (
