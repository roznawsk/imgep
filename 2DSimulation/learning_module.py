...
class LearningModule(object):
    def __init__(
        ...
        conf = make_configuration(
            ...
            list(
                np.array(list(env_conf.m_mins[m_space]) + list(env_conf.s_mins))[
                    s_space
                ]
            )
            * self.max_steps,
            list(
                np.array(list(env_conf.m_maxs[m_space]) + list(env_conf.s_maxs))[
                    s_space
                ]
            )
            * self.max_steps,
        ...
        if self.end_point:
            self.sm_end = BufferedDataset(
                conf.m_ndims, len(s_space), buffer_size=10000, lateness=100
            )
            ...
            self.interest_model = MiscRandomInterest(
                conf, conf.s_dims, self.n_sdims, win_size=200
            )

    def motor_babbling(self, steps=None):
        return np.random.random(self.n_mdims * self.max_steps) * 2.0 - 1.0
        ...
        if explore:
            if self.optim_explo == "gaussian" or self.optim_explo == "random":
                ...
                if self.optim_explo == "gaussian":
                    explo_vect = [0.0] * start_explo * self.n_mdims + [
                        self.explo_noise
                    ] * (snn_steps - start_explo) * self.n_mdims
                    m = np.random.normal(m, explo_vect).clip(-1.0, 1.0)
                else:
                    rdm = 2.0 * np.random.random(self.max_steps * self.n_mdims) - 1.0
    ...
    def update_im(self, m, s):
        if self.t > self.motor_babbling_n_iter:
            if self.end_point:
                return self.interest_model.update(
                    self.sg[-len(self.s_space) :], s[-len(self.s_space) :]
                )
            else:
                return self.interest_model.update(self.sg, s)

    def interest(self):
        return self.interest_model.current_interest

    def perceive(self, m, s):
        self.update_sm(m, s)

    def get_m(self, ms):
        return ms[self.m_space]

    def get_s(self, ms):
        return ms[self.s_space]
